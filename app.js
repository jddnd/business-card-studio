(function () {
  ...
  // ✅ Safe fallback if Framer Motion isn't available
  ...
  function App() {
                ...
                {order.status === 'Pending' && (
                  <>
                    <textarea
                      ...
                      className="w-full p-3 mt-2 bg-white/50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-300"
                    />
                    <motion.button
                      whileHover={{ scale: 1.05 }}
                      whileTap={{ scale: 0.95 }}
          ...
          <ul className="space-y-4">
            {employees.map(employee => (
              <motion.li key={employee.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white/30 p-4 rounded-xl">
                ...
                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => shareCard(employee)} className="mt-2 w-full bg-blue-500 text-white p-3 rounded-xl hover:bg-blue-600 transition">
                  Share Card
                </motion.button>
              </motion.li>
            ))}
    ...
    return (
      React.createElement(
        'div',
        { className: 'min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-white p-6 flex flex-col items-center' },
        ...
        React.createElement(motion.h1, {
          ...
          className: 'text-4xl font-sans font-semibold text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-indigo-600 mb-8'
        ...
        React.createElement(motion.div, {
          ...
          React.createElement('input', {
            ...
            className: 'w-full p-3 mb-3 bg-white/50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-300'
          }),
          React.createElement(motion.button, {
            whileHover: { scale: 1.05 },
            whileTap: { scale: 0.95 },
            onClick: receiveCard,
            className: 'w-full bg-indigo-500 text-white p-3 rounded-xl hover:bg-indigo-600 transition'
          }, 'Receive Card')
        )
      )
    );
